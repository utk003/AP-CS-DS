    ...
    private void createAndShowGUI()
    {
        ...
        for (int row = 0; row < grid.length; row++)
        	for (int col = 0; col < grid[row].length; col++)
        	{
				...
				grid[row][col].setPreferredSize(new Dimension(50 * mult, 50 * mult));
		...
		for (int row = 0; row < grid.length; row++)
			for (int col = 0; col < grid[row].length; col++)
			{
				...
				if (piece != null)
				{
					...
					ImageIcon imageIcon = new ImageIcon(piece.getImageFileName());
					int x = mult * imageIcon.getIconWidth(), y = mult * imageIcon.getIconHeight();
					icon = new ImageIcon(imageIcon.getImage().getScaledInstance(x, y, Image.SCALE_DEFAULT));
				}
